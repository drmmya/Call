...
async function endCall(callId){ const r=await db.query("UPDATE calls SET status='ended', ended_at=NOW() WHERE id=$1 RETURNING *",[callId]);
...
setInterval(async ()=>{ const keys=await redis.keys("call:*"); for(const k of keys){ const meta=await redis.hGetAll(k);
  if(!meta || meta.status!=="active") continue; const callId=k.split(":")[1];
  const r=await db.query("SELECT * FROM calls WHERE id=$1 AND status='active'",[callId]); if(!r.rowCount){ await redis.hSet(k,{status:"ended"}); continue; }
  const {caller_id,callee_id,kind}=r.rows[0]; const cost=COST[kind]||10; const [a,b]=await Promise.all([getCoins(caller_id),getCoins(callee_id)]);
  if(a<cost||b<cost){ await endCall(callId); continue; } await addCoins(caller_id,-cost); await addCoins(callee_id,-cost);
  const sa=userToSocket.get(caller_id), sb=userToSocket.get(callee_id); if(sa) io.to(sa).emit("wallet:update",{coins:await getCoins(caller_id)}); if(sb) io.to(sb).emit("wallet:update",{coins:await getCoins(callee_id)}); }
},60_000);
